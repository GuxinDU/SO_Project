...
class AgentPointEstimation:
    def _orca_lines_point_estimation(self, sample_budget=10):
        """
        ...
        for dist_sq, other in self.agent_neighbors:
            ...
            average_error = np.array([0.0, 0.0])
            for _ in range(sample_budget):
                error = self.error_generator.sample()
                average_error += error
            average_error /= sample_budget

            relative_velocity += Vector2(average_error[0], average_error[1])
