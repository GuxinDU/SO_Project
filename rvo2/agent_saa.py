...
class AgentSAA:
    def _orca_lines_saa(self, sample_budget=10):
        """
        ...
        for dist_sq, other in self.agent_neighbors:
            ...
            nominal_relative_velocity = self.velocity - other.velocity
            combined_radius = self.radius + other.radius
            combined_radius_sq = sqr(combined_radius)

            for _ in range(sample_budget):
                error = self.error_generator.sample()
                relative_velocity = nominal_relative_velocity + Vector2(error[0], error[1])
